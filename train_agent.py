...
def train_agent():
    ...
    # 학습 이어하기(모델 저장 파일 로드)
    model_path = f'models/best_model_DQN{cf.DQN_VER}_{cf.NUM_BUFFER}Buffer.pth'
    ...
    for episode in range(_NUM_EPISODES):
        ...
        if frame_since_update > _UPDATE_FREQ and episode_since_update >= 3:
            target_model.load_state_dict(model.state_dict())
            print(f'타겟 모델 업데이트 after {episode_since_update}episodes, {frame_since_update}frames')
            frame_since_update = 0
            episode_since_update = 0
        # 생존시간
